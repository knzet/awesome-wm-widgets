...
-- load the module for brightness, volume, battery widget
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({"home", "ref", "music", "misc"}, s, awful.layout.layouts[1])
...
-- {{{ Key bindings
globalkeys = gears.table.join(awful.key({}, "#233", function()
    local cmds = {"nitrogen --restore", "picom"}
...
}, NN -- Set Firefox to always map on the tag named "2" on screen 1.
-- { rule = { class = "Firefox" },
...
local cmds = {"compton", "xautolock -time 7 -locker lock &"}
...
-- i think the loop is async so do this after the loop is done
-- also, these commands need shell context
awful.util.spawn_with_shell("sleep 0.3 && nitrogen --restore")
awful.util.spawn_with_shell('redshift -x && redshift -O 3000 -b 0.7')

-- garbage collector or something so lua stops eating all my ram
